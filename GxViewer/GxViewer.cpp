...
CGxViewer::CGxViewer(QWidget *parent) :
    ...
    m_pobjShowFrameRateTimer(NULL),
      ...
      m_pobjROISettings2(NULL),
      m_pobjFrameRateControl2(NULL),
      m_pobjExposureGain2(NULL),
      m_pobjWhiteBalance2(NULL),
      m_pobjImgProc2(NULL),
      m_pobjUserSetCtl2(NULL),
      m_hDevice2(NULL),
      m_pstBaseInfo2(NULL),
      m_ui32DeviceNum2(0),
      m_bOpen2(false),
      m_bAcquisitionStart2(false),
      m_bTriggerModeOn2(false),
      m_bSoftTriggerOn2(false),
      ...
      m_pobjShowFrameRateTimer2(NULL)
{
    ...
    connect(m_pobjShowImgTimer2, SIGNAL(timeout()), this, SLOT(slotShowImage()));
    connect(m_pobjShowFrameRateTimer2, SIGNAL(timeout()), this, SLOT(slotShowFrameRate()));
...
void CGxViewer::UpdateUI()
{
    ...
    ui->OpenDevice2->setEnabled(ui->DeviceList2->count() > 0 && !m_bOpen);
...
void CGxViewer::SetUpDialogs()
{
    ...
    // Connect image processing parameters to acquisition-thread class
    connect(m_pobjImgProc2, SIGNAL(SigSendColorCorrectionParam2(int64_t)),
            m_pobjAcqThread2, SLOT(SlotGetColorCorrectionParam2(int64_t)));
    connect(m_pobjImgProc2, SIGNAL(SigSendGammaLUT2(unsigned char*)),
            m_pobjAcqThread2, SLOT(SlotGetGammaLUT2(unsigned char*)));
    connect(m_pobjImgProc2, SIGNAL(SigSendContrastLUT2(unsigned char*)),
            m_pobjAcqThread2, SLOT(SlotGetContrastLUT2(unsigned char*)));
    ...
    connect(m_pobjUserSetCtl2, SIGNAL(SigRefreshMainWindow2()),
            this, SLOT(slotRefreshMainWindow2()));
...
void CGxViewer::OpenDevice2()
{
    ...
    emStatus2 = GXOpenDeviceByIndex(ui->DeviceList2->currentIndex() + 1, &m_hDevice2);
    GX_VERIFY(emStatus2);
...
void CGxViewer::CloseDevice2()
{
    ...
    //Close Device
    emStatus2 = GXCloseDevice(m_hDevice2);
    GX_VERIFY(emStatus2);
...
*/
//----------------------------------------------------------------------------------
void CGxViewer::ShowDeviceInfo2()
{
    GX_STATUS emStatus2 = GX_STATUS_SUCCESS;
    // 128 bytes for device info string is enough
    const int nStringLength = 128;
    ...
    char arrVendorNameString[nStringLength];
    nSize = sizeof(arrVendorNameString);
    emStatus2 = GXGetString(m_hDevice2, GX_STRING_DEVICE_VENDOR_NAME, arrVendorNameString, &nSize);
    GX_VERIFY(emStatus2);
    // Show device information on Info label
    ui->VendorName->setText(QString("VendorName: %1").arg(QString(arrVendorNameString)));
    ...
    char arrModelNameString[nStringLength];
    nSize = sizeof(arrModelNameString);
    emStatus2 = GXGetString(m_hDevice2, GX_STRING_DEVICE_MODEL_NAME, arrModelNameString, &nSize);
    GX_VERIFY(emStatus2);
    // Show device information on Info label
    ui->ModelName->setText(QString("ModelName: %1").arg(QString(arrModelNameString)));
    ...
    char arrSerialNumberString[nStringLength];
    nSize = sizeof(arrSerialNumberString);
    emStatus2 = GXGetString(m_hDevice2, GX_STRING_DEVICE_SERIAL_NUMBER, arrSerialNumberString, &nSize);
    GX_VERIFY(emStatus2);
    // Show device information on Info label
    ui->SerialNumber->setText(QString("SN: %1").arg(QString(arrSerialNumberString)));
    ...
    char arrDeviceVersionString[nStringLength];
    nSize = sizeof(arrDeviceVersionString);
    emStatus2 = GXGetString(m_hDevice2, GX_STRING_DEVICE_VERSION, arrDeviceVersionString, &nSize);
    GX_VERIFY(emStatus2);
    // Show device information on Info label
    ui->DeviceVersion->setText(QString("DeviceVersion: %1").arg(QString(arrDeviceVersionString)));
...
bool CGxViewer::SetAcquisitionBufferNum2()
{
    ...
    // Get device current payload size
    emStatus2 = GXGetInt(m_hDevice2, GX_INT_PAYLOAD_SIZE, &i64PayloadSize);
    if (emStatus2 != GX_STATUS_SUCCESS)
    {
        ShowErrorString(emStatus2);
        return false;
    }
    ...
    emStatus2 = GXSetAcqusitionBufferNumber(m_hDevice2, ui64BufferNum);
    if (emStatus2 != GX_STATUS_SUCCESS)
    {
        ShowErrorString(emStatus2);
        return false;
...
void CGxViewer::GetDeviceInitParam2()
{
    ...
    // Init trigger mode combobox entrys
    emStatus2 = InitComboBox(m_hDevice2, ui->TriggerMode, GX_ENUM_TRIGGER_MODE);
    if (emStatus2 != GX_STATUS_SUCCESS)
    {
        CloseDevice();
        GX_VERIFY(emStatus2);
    ...
    // Init trigger source combobox entrys
    emStatus2 = InitComboBox(m_hDevice2, ui->TriggerSource, GX_ENUM_TRIGGER_SOURCE);
    if (emStatus2 != GX_STATUS_SUCCESS)
    {
        CloseDevice();
        GX_VERIFY(emStatus2);
    ...
    // Device support frame control or not
    bool bFrameRateControl = false;
    emStatus2 = GXIsImplemented(m_hDevice2, GX_ENUM_ACQUISITION_FRAME_RATE_MODE, &bFrameRateControl);
    if (emStatus2 != GX_STATUS_SUCCESS)
    {
        CloseDevice();
        GX_VERIFY(emStatus2);
    ...
    // Device is a color-camera or not
    bool bColorFilter = false;
    emStatus2 = GXIsImplemented(m_hDevice2, GX_ENUM_PIXEL_COLOR_FILTER, &bColorFilter);
    if (emStatus2 != GX_STATUS_SUCCESS)
    {
        CloseDevice();
        GX_VERIFY(emStatus2);
...
*/
//----------------------------------------------------------------------------------
bool CGxViewer::CheckMultiROIOn2()
{
    GX_STATUS emStatus2 = GX_STATUS_SUCCESS;

    int64_t emRegionSendMode = GX_REGION_SEND_SINGLE_ROI_MODE;
    bool bRegionMode = false;
    emStatus2 = GXIsImplemented(m_hDevice2, GX_ENUM_REGION_SEND_MODE, &bRegionMode);
    if (emStatus2 != GX_STATUS_SUCCESS)
    {
        ShowErrorString(emStatus2);
    }
    ...
    if (bRegionMode)
    {
        emStatus2 = GXGetEnum(m_hDevice2, GX_ENUM_REGION_SEND_MODE, &emRegionSendMode);
        if (emStatus2 != GX_STATUS_SUCCESS)
        {
            ShowErrorString(emStatus2);
        }
...
void CGxViewer::StartAcquisition2()
{
    ...
    emStatus2 = GXStreamOn(m_hDevice2);
    GX_VERIFY(emStatus2);
...
void CGxViewer::on_StopAcquisition2_clicked()
{
    ...
    // Reset acquisition frame rate to 0
    ui->AcqFrameRateLabel->setText(QString("Frame NUM: %1   Acq. FPS: %2")
                                   .arg(m_pobjAcqThread2->m_nFrameCount)
                                   .arg(0.0, 0, 'f', 1));
    // Reset display frame rate
    ui->ShowFrameRateLabel->setText(QString("Disp. FPS: %1").arg(0.0, 0, 'f', 1));
...
void CGxViewer::slotShowImage2()
{
    ...
    // Display the image
    QImage objImgScaled2 = qobjImgShow2->scaled(ui->ImageLabel_2->width(), ui->ImageLabel->height(),
                                           Qt::IgnoreAspectRatio, Qt::FastTransformation);
    ui->ImageLabel_2->setPixmap(QPixmap::fromImage(objImgScaled2));
